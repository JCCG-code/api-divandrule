...
export const getTransactions = (req, res) => {
  const transactions = Transaction.find({}, async (err, transactions) => {
    ...
    for (const transaction of transactions) {
      ...
      let contract = await Asset.findOne({
      ...
      if (contract) {
        ...
        let auxDateRegister = new Date(
          transactionsMap[transaction._id].dateRegister
        );
        ...
        if (contract.type_of_payment == "monthly") {
          while (datesClaim[i].date < contract.dateFinish) {
            ...
            // donde el usuario puede reclamar sus tokens
              ...
              if (
                transactionsMap[transaction._id].datesClaim[i].isClaim ==
                "Claimed"
              ) {
                auxIsClaim = "Claimed";
              } else {
                auxIsClaim =
                  ...
                  parseFloat(
                    (
                      investmentAmount * (1 + interestPerMonth / 100) -
                      investmentAmount
                    ).toFixed(4)
                  );
                cont++;
          ...
          let newDate = {
            date: auxDate,
            isClaim: auxIsClaim,
            nMonth: contract.monthsDuration,
          };
          datesClaim.push(newDate);
        ...
        await Transaction.updateOne(
          { _id: transactionsMap[transaction._id]._id },
          {
            datesClaim: datesClaim,
          }
        );
